...
//  if (preg_match('#First#', $type)) $type .="RM";
            ...
            // -----
            // Give an observer the opportunity to modify handling costs.
            //
            $this->notify('NOTIFY_SHIPPING_USPS_AFTER_HANDLING', array(), $order, $usps_shipping_weight, $shipping_num_boxes, $usps_handling_fee);
        ...
        foreach ($transresp as $service => $val) {
            ...
            switch (true) {
                case (stripos($service, 'Priority Mail Express') !== false):
                    ...
                    break;
                case (stripos($service, 'Priority MailTM') !== false):
                    $time = $val['Days'];
                    if ($time == '' || $time == 'No Data') {
